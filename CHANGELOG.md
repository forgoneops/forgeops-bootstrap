...
- `CLAUDE.md` "Isolated-project Compose testing (validation runs)" section: documents and enforces that isolated-project Compose testing must never pass `-v`/`--volumes` to `docker compose down` against this repo's `docker-compose.yml`. `docker-compose.yml`'s `volumes:` block pins literal names (not `${COMPOSE_PROJECT_NAME}`-scoped), so `-p <isolated-project> down -v` can target production's real named volumes even when production's services were never part of that isolated run — Compose project-name scoping doesn't protect volumes here the way it protects container/network names. Confirmed via a dry-run `docker compose config` on the VPS clone that dropping the literal `name:` overrides would derive different volume names (`forgeops-bootstrap_*` vs. production's existing `forgeops_*`) and orphan production's data on the next `install.sh`/`update.sh` run, so the literal names stay; the hard rule (never `-v` against this compose file) is the enforced mitigation instead. No code changes.
