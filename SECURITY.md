...
- **`mcp-stdio-bridge`'s exact `mcp-proxy` CLI invocation** (wrapping a stdio MCP server behind `--sse-path`) matches the general convention for this class of tool but wasn't re-confirmed against `mcp-proxy --help` this session — verify before first production deploy. Every build-time input to that bridge (`PYTHON_BASE_IMAGE`, `MCP_PROXY_VERSION`, and per-service `MCP_SERVER_VERSION`) is now pinned in `configs/versions.env`; a previous `MCP_PROXY_IMAGE` build-arg was passed but never declared or used by the Dockerfile, so despite the name nothing was actually pinned. One exception: `mcp-git`'s `MCP_SERVER_VERSION` (`MCP_GIT_SERVER_VERSION` in `configs/versions.env`) is deliberately left blank — `@modelcontextprotocol/server-git` returned 404 from the npm registry this session, i.e. that package doesn't appear to exist. The official git MCP server ships on PyPI/uvx as `mcp-server-git`, not npm, so `mcp-git` was likely already broken (crash-looping) before this pinning fix and needs its own follow-up — either a corrected package name or a uvx-based bridge variant — tracked separately from version pinning.
