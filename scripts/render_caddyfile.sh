#!/usr/bin/env bash
# render_caddyfile.sh - generates docker/Caddyfile from templates/Caddyfile.template
# based on the current .env (DOMAIN, EXPOSE_PORTAINER, EXPOSE_UPTIME_KUMA).
#
# Called by install.sh and update.sh. Safe to re-run at any time.
...
[[ -f "${ENV_FILE}" ]] || die "render_caddyfile.sh: .env not found — run install.sh first."
