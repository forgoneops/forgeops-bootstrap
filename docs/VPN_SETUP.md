...
**Confirmed against wg-easy's own v15 docs (see commit message for source URLs; sourced from the `v15.2` doc tree, closest published version to the pinned `15.3.0` image — not independently re-checked against `15.3.0` specifically): there is no CLI or pre-authenticated API for creating a peer.** Its `/api` requires the same admin login as the web UI — no documented creation endpoint exists that would bypass it, though upstream has no explicit "unsupported" statement to that effect either. It can't bootstrap a session that doesn't exist yet. That makes the very first peer a genuine chicken-and-egg problem: the only way to create it is through a UI that, by this stack's design, isn't reachable until a tunnel already exists. There's no way around this — the practical fix is a one-time manual bootstrap:
